...
| `trigger-ux` | 판단은 맞았는데 **자동화 계층이 사용자에게 노출되는 방식**이 문제 | `hooks/` |
...
### 2026-07-22 — v0.4 Stop 훅: 실전 1분 만에 롤백
- 상황: 훅을 등록하고 재시작한 직후
- 엔진 동작: 훅 발동 → 제안까지 정상 도달 (게이트·판단·분류 모두 의도대로)
- 기대: 조용한 경로에서는 사용자가 아무것도 못 느껴야 함
- 실제: **`decision: block`의 `reason`이 사용자 화면에 에러처럼 그대로 노출.**
  "훅이 돌았다는 말도 하지 마라"는 지시문 자체가 매번 화면에 찍힘 → 침묵 경로가 구조적으로
  존재하지 않음. 추가로 백그라운드 잡과 인터랙티브 세션이 서로 다른 `session_id`라
  "세션당 1회" 가드가 사용자 체감으로는 반복이 됨 (마커 UUID 2개 확인)
- 범주: `trigger-ux` (신설 — 판단이 아니라 **자동화 계층의 노출 방식**이 문제)
- 조치: `hook-wiring.py remove`로 등록 해제. 코드는 main에 유지(PR #2), 게이트 테스트 19개는
  여전히 통과 — 게이트는 멀쩡하고 "차단으로 트리거한다"는 방식이 문제
- 후속 검토: Stop 차단 말고 노출이 없는 트리거가 있는가? 없다면 자동 트리거 자체를 접고
  `/document` 수동 호출 + 세션 종료 시 가벼운 리마인더 쪽으로 방향 전환 검토
