...
<!-- 스니펫은 이 문서만 보고 재현 가능해야 한다.
     등장하는 상수·설정값은 반드시 실제 값을 함께 적는다 (이름만 두지 말 것).
     값이 여럿이면 상수 블록을 로직 위에 따로 둔다. -->
...
### 이 숫자들은 어떻게 정했나
튜닝 상수마다: **의미(단위/정의)** · **정한 방법**(측정 / 계산 / 눈으로 맞춤 — 감이면 감이라고
쓴다) · **바꾸면 뭐가 깨지는지**(동작하는 범위, 벗어나면 나타나는 증상).
서로 물려 있는 값이 있으면 (A를 바꾸면 B도 다시 계산해야 함) 그 의존 관계를 명시한다.
모르는 값은 지어내지 말고 `(미확인)`.
